...
final class CommercePayTransactionFilterTag extends StringValueObject
{
    ...
    public  function cardBrandIs(?string $value):string
    {
        ...
        if ($value === self::MASTER_CARD_BRAND) $cardBrand = $value;

        if ($value === self::VISA_CARD_BRAND) $cardBrand = $value;

        if ($value === self::AMEX_CARD_BRAND) $cardBrand = $value;

        if ($value === self::CARNET_CARD_BRAND) $cardBrand = $value;
